...
class animation :
    ...
    def plot_3D(self ):
        ...
        po, = axis.plot(0,0,0,'bo')
        p1, = axis.plot(0,0,0,'go')
        p2, = axis.plot(0,0,0,'go')
        p3, = axis.plot(0,0,0,'ro')
        ...
        def animate(i) :
            #pg, = axis.plot(2,2,1,'go')
            po, = axis.plot(0,0,0,'bo')
            p1, = axis.plot(self.x1[i][1],self.y1[i][1],self.z1[i][1],'g.')
            p2, = axis.plot(self.x2[i][1],self.y2[i][1],self.z2[i][1],'g.')
            p3, = axis.plot(self.x3[i][1],self.y3[i][1],self.z3[i][1],'r.')
            line1, = axis.plot(self.x1[i], self.y1[i],self.z1[i],lw = 3,color='yellow') 
            line2, = axis.plot(self.x2[i], self.y2[i],self.z2[i],lw = 3,color='yellow') 
            line3, = axis.plot(self.x3[i], self.y3[i],self.z3[i],lw = 3,color='yellow')
    ...
    def plot_3D_gif(self ,name):
        ...
        line1, = axis.plot([], [],[], lw = 3,color='purple')  
        line2, = axis.plot([], [],[], lw = 3,color='purple')  
        line3, = axis.plot([], [],[], lw = 3,color='purple')  
        po, = axis.plot([0],[0],[0],'b.')
        p1, = axis.plot([0],[0],[0],'r.')
        p2, = axis.plot([0],[0],[0],'r.')
        p3, = axis.plot([0],[0],[0],'r.')
        pg, = axis.plot([-2],[-1],[-1],'go')
        ...
        for i in range(len(self.x1)):
            ...
            pg, = axis.plot([-2],[-1],[-1],'go')
            po, = axis.plot([0],[0],[0],'bo')
            p1, = axis.plot([self.x1[i][1]],[self.y1[i][1]],[self.z1[i][1]],'r.')
            p2, = axis.plot([self.x2[i][1]],[self.y2[i][1]],[self.z2[i][1]],'r.')
            p3, = axis.plot([self.x3[i][1]],[self.y3[i][1]],[self.z3[i][1]],'r.')
            camera.snap()
